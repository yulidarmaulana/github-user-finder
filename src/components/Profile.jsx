        ...
        <div className="flex flex-col mr-3 lg:flex-row lg:items-baseline lg:justify-between">
          <div>
            <p
              ...
              } font-bold text-lg 480:text-2xl -mb-2 480:mb-1`}
            >
              {prop.profile.name == null
                ? prop.profile.login
                : prop.profile.name}
            </p>
            <a
              href={`https://github.com/${prop.profile.login}`}
              target="_blank"
              rel="noreferrer"
        ...
        <div className="mt-6 480:mt-10 -ml-20 text-xs 480:text-base lg:ml-0 lg:mt-3">
          ...
          >
            <div className="flex flex-col ">
              <p className={`${prop.dark ? "text-white" : "text-gray"}`}>
                Repos
              </p>
              <p
                className={`font-bold text-lg 480:text-2xl mx-auto sm:mx-0 ${
                  prop.dark ? "text-white" : "text-dark-navy-blue-2"
                }`}
              >
                {prop.profile.public_repos}
              </p>
            </div>
            <div className="flex flex-col">
              <p className={`${prop.dark ? "text-white" : "text-gray"}`}>
            ...
            <div className="flex flex-col mt-8 600:grid 600:grid-cols-2 600:break-all lg:break-normal">
              <div className="flex items-center mb-4 gap-x-5 ">
                ...
                {prop.profile.location != null ? (
                  <p
                    className={`${
                      prop.dark ? "text-white" : "text-grayish-blue"
                    }`}
                  >
                    {prop.profile.location}
                  </p>
                ) : (
                  <p
                    className={`${
                      prop.dark ? "text-gray" : "text-navy-blue-2"
                    }`}
                  >
                    Not Available
                  </p>
              ...
              <div className="flex items-center mb-4 gap-x-4 ">
                <svg
                  ...
                  className={`${
                    prop.dark
                      ? prop.profile.twitter_username != null
                        ? "fill-white"
                        : "fill-gray"
                      : prop.profile.twitter_username != null
                      ? "fill-grayish-blue"
                ...
                {prop.profile.twitter_username != null ? (
                  <a
                    href={`https://twitter.com/${prop.profile.twitter_username}`}
                    target="_blank"
                    rel="noreferrer"
